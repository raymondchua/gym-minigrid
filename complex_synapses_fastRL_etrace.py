...
def compute_SF_td_error(state, action, next_state, next_action, SF_values):
	ind_func = np.zeros((grid_size*grid_size))
	ind_func[next_state] = 1 
...
def main():
	...
	for epoch in range(num_epochs):
		...
		for episode in range(num_episodes):
			...
			etrace = np.zeros((len(env.actions), grid_size*grid_size)) 
			...
			for time_steps in range(max_steps):
				...
				sf_error = np.squeeze(compute_SF_td_error(state, action, next_state, next_action, SF_u1))

				etrace = np.multiply(etrace,discount * lam_factor)
				etrace[action, state] = 1
				sf_error_etrace = np.multiply(etrace, sf_error[next_state])

				# print('sf error: ', sf_error)
				# print('sf etrace: ', sf_error_etrace[action,:])


				SF_u1[state, action,:] = SF_u1[state, action,:] + ((sf_lr/C_1) * (sf_error + g_1_2 * (SF_u2[state, action, :] - SF_u1[state, action, :])))

				# SF_u1[state, action, next_state] = SF_u1[state, action, next_state] + ((sf_lr/C_1) * (sf_error[next_state] + g_1_2 * (SF_u2[state, action, next_state] - SF_u1[state, action, next_state])))


				# #update SF_u2 using SR-td error
				SF_u2 = SF_u2 + ((sf_lr/C_2) * (g_1_2 * (SF_u1 - SF_u2) + \
								g_2_3*(SF_u3 - SF_u2)))
				...
				# SF_u1[state,:] = SF_u1[state,:] + ((sf_lr/C_1) * (sf_error_etrace + g_1_2 * (SF_u2[state,:] - SF_u1[state,:])))
				...
				if steps_done % TIMESTEP_WINDOW == 0:
					...
					if args.save_np_files: 
						...
						filename_SF_u2 = 'fastRL_SF_u2_'+file_index_pad+'.npy'
						filename_SF_u3 = 'fastRL_SF_u3_'+file_index_pad+'.npy'
						...
						filepath_SF_u2 = os.path.join(SF_path, filename_SF_u2)
						utils.create_folders_if_necessary(filepath_SF_u2)

						filepath_SF_u3 = os.path.join(SF_path, filename_SF_u3)
						utils.create_folders_if_necessary(filepath_SF_u3)
						...
						np.save(filepath_SF_u2, SF_u2)
						np.save(filepath_SF_u3, SF_u3)
